...
$$
r_{i}=\operatorname{Pr}\left( i \text{correctly linked with} i^* \mid s , P \text{, worst case scenario }\right) \qquad\text{(3.2.3.1)}
$$
...
$$
r_{i} = \mathrm{E} \left( \frac{1}{F_{k}} \mid f_{k} \right) = \sum\limits_{h\geq f_{k}} \frac{1}{h} \operatorname{Pr} \left(F_{k} = h \mid f_{k} \right). \qquad\text{(3.2.5.1)}
$$
