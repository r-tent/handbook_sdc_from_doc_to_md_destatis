...
Values of (\ref{mgamma}) are also easily interpretable, as the expected loss of information about connections between variables and can be expressed in $\%$. Of course, both matrices must be based on the same correlation coefficient. The most obvious choice in this respect is the Pearson's index. However, when tau-Kendall correlation matrix is used, one can also apply it to ordinal variables. The method will be not applicable if the correlation matrix is singular. The measure $\gamma$ can be computed in the sdcMicro R package using the function IL_correl().  
