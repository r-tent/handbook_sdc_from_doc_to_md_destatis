...
Source: Domingo-Ferrer, Mateo-Sanz and Torra (2001) and Domingo-Ferrer and V. Torra (2001).
...
$$
d({x_{ij}}^',x_{hj})=\begin{cases}
1&\textmd{if}\;x_{ij}}^'=x_{hj},\cr
0&\textmd{if}\;x_{ij}}^'\ne x_{hj}.
$$
If $X_j$ is ordinal, then (NA is treated as a separate, lowest category)
$$
d(x_{ij}}^',x_{hj})=\frac{\mathfrak{r}(x_{ij}}^',x_{hj})}{\mathfrak{r}_j-1},
$$
where $\mathfrak{r}(x_{ij}}^',x_{hj})$ is the absolute difference in categories between $x_{ij}}^'$ and $x_{hj}$ and $\mathfrak{r}_j$ is the total number of categories of $X_j$.These partial distances take always values from [0,1]. There are, however, some problems with using these measures. One of the SDC tools for categorical variables is recoding. The number of categories of such a recoded variable in the original set and in the new SDC resulting from it will be different. Therefore, in the first place, it should be ensured that the numbers of the categories left unchanged are identical in both variants. For example, if before recoding the variable $X_j$ had $k_j$=8 categories marked as 1,2,3,4,5,6,7,8, and as a result of recoding categories 2 and 3 and 6 and 7 were combined, then the new categories should have respectively numbers 1,2,4,5,6,8. Then approach (3.5.2) applies in this case as well.
...
$$d(x_ij,x_ij^* )=|x_ij-x_ij^* |/max┬(k=1,2,…,n)⁡|x_kj-x_kj^* |,$$                                                    (4)
lub
d(x_ij,x_ij^* )=(x_ij-x_ij^* )^2/max┬(k=1,2,…,n)⁡〖(x_kj-x_kj^* )^2 〗 ,                                                 (5)
i=1,2,…,n, j=1,2,…,m.

Measures (4) and (5) also have another significant weakness. The measure of information loss should be an increasing function due to individual partial information losses. This means that, for example, if for some i∈{1,2,,…,n} the value |x_ij-x_ij^* | will increase and all |x_hj-x_hj^* | for h≠i remain the same, the value of the gauge should increase. Meanwhile, in the case of formulas (4) and (5), this will not be the case. If, for the same, the indicated absolute difference (or the square of the difference, respectively) between the original value and the value after SDC reaches a maximum, then the partial loss of information for i will remain unchanged - it will be 1, and for the others it will turn out to be smaller. As a result, we get a smaller metric value, while the information loss actually increased.
...
The complex measure of distribution disturbance is given by (cf. Młodak, Pietrzak and Józefowksi (2022)):
\begin{equation}
\lambda=\sum_{j=1}^m{\sum_{i=1}^n{\frac{d(x_{ij},x_{ij}^*)}{mn}}}\in [0,1],
\label{mlambda}
\end{equation}
where $d(\cdot,\cdot)\in [0,1]$ is measure of distance.
...
The measure (\ref{mlambda}) can be expressed as a percentage and shows total information loss -- the greater the value of $\lambda$, the bigger the loss. In this way users obtain clear and easily understandable information about expected information loss owing to the application of SDC. 
IL_variables
