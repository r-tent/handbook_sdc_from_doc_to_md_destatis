...
1, & \text{if $c\neq c'$},
\end{cases}$$ (3.5.1)
where $c$ is a category in an original record and $c^{\prime}$ is the category which has replaced *c* in the corresponding protected record.
...
$$\text{dc}\left(c,c^{\prime}\right)=\frac{\left|c^{\prime\prime}\text{:min}\left(c,c^{\prime}\right)\leq c^{\prime\prime}<\text{max}\left(c,c^{\prime}\right)\right|}{\left|D(V)\right|}$$ (3.5.2)
:::
...
$$\text{CTBIL}(F,G;W,K)=\sum_{\{V_{ji}\cdots V_{jt}\} f\subseteq W\atop|\{V_{ji}\cdots V_{jt}\}|\leq K}\sum_{i_1\cdots i_t}|x^F_{i_1\cdots i_t}-x^G_{i_1\cdots i_t} |$$(3.5.3)

where $x_{\text{subscripts}}^{\text{file}}$ is the entry of the contingency table of $\text{file}$ at position given by $\text{subscripts}$.

Because the number of contingency tables to be considered depends on the number of variables $|W|$, the number of categories for each variable, and the dimension $K$, a normalized version of Expression (3.5.3) may be desirable. This can be obtained by dividing Expression (3.4.2.1) by the total number of cells in all considered tables.
...
Let $V$ be a variable in the original dataset and $V^{\prime}$ be the corresponding variable in the PRAM-protected dataset. Let $P_{V,V^{\prime}}=\left\{p\left(V^{\prime}=j|V=i\right) \right\}$ be the PRAM Markov matrix. Then, the conditional uncertainty of $V$ given that $V^{\prime}=j$ is:

$H\left(V|V^{\prime}=j\right)=-\sum_{i=1}^n{p\left(V=i|V^{\prime}=j\right)\text{log}p}\left(V=i|V^{\prime}=j|\right)$ (3.5.4)

The probabilities in Expression (3.5.4) can be derived from $P_{V,V^{\prime}}$ using Bayes's formula. Finally, the entropy-based information loss measure *EBIL* is obtained by accumulating Expression (3.5.4) for all individuals $r$ in the protected dataset $G$

$\text{EBIL}\left(P_{V,V^{\prime}},G\right)=\sum_{r\in G}{H\left(V|V^{\prime}=j_{r}\right)}$

where $j_r$ is the value taken by $V^{\prime}$ in record $r$.
...
• covariance matrices $V$ (on $X$) and $V^{\prime}$ (on $X^{\prime}$).
• correlation matrices $R$ and $R^{\prime}$.
• correlation matrices $\text{RF}$ and $RF^{\prime}$ between the $p$ variables and the $p$ factors principal components $\text{PC}_1,\text{PC}_2,\cdots,\text{PC}_p$ obtained through principal components analysis.
• communality between each of the $p$ variables and the first principal component $\text{PC}_{1}$ (or other principal components $\text{PC}_i$*'s*). Communality is the percent of each variable that is explained by $\text{PC}_{1}$ (or $\text{PC}_i$). Let $C$ be the vector of communalities for $X$ and $C^{\prime}$ the corresponding vector for $X^{\prime}$.
• matrices $F$ and $F^{\prime}$ containing the loadings of each variable in $X$ on each principal component. The i-th variable in $X$ can be expressed as a linear combination of the principal components plus a residual variation, where the $j$-th principal component is multiplied by the loading in $F$ relating the $i$-th variable and the $j$-th principal component (Chatfield and Collins, 1980\]. $F^{\prime}$ is the corresponding matrix for $X^{\prime}$.

There does not seem to be a single quantitative measure which completely reflects those structural differences. Therefore, we proposed in Domingo-Ferrer, Mateo-Sanz, and Torra (2001) and Domingo-Ferrer and Torra (2001) to measure information loss through the discrepancies between matrices $X$, $V$, $R$, $\text{RF}$, $C$ and $F$ obtained on the original data and the corresponding $X^{\prime}$, $V^{\prime}$, $R^{\prime}$, $RF^{\prime}$, $C^{\prime}$ and $F^{\prime}$ obtained on the protected dataset. In particular, discrepancy between correlations is related to the information loss for data uses such as regressions and cross tabulations.
...
Table 1 summarizes the measures proposed in Domingo-Ferrer, Mateo-Sanz and Torra (2001) and Domingo-Ferrer and V. Torra (2001). In this table, $p$ is the number of variables, $n$ - the number of records, and components of matrices are represented by the corresponding lowercase letters (*e.g.* $x_{\text{ij}}$ is a component of matrix $X$). Regarding $X-X^{\prime}$ measures, it makes also sense to compute those on the averages of variables rather than on all data (call this variant $\overline{X} - \overline{X^{\prime}}$. Similarly, for $V-V^{\prime}$ measures, it would also be sensible to use them to compare only the variances of the variables, *i.e.* to compare the diagonals of the covariance matrices rather than the whole matrices (call this variant $S-S^{\prime}$).
...
$$
\lambda=\sum_{j=1}^m{\sum_{i=1}^n{\frac{d(x_{ij}^{\prime},x_{ij})}{mn}}}\in [0,1],   (3.5.12)
$$
where $d(\cdot,\cdot)\in [0,1]$ is measure of distance according to the formulas (3.5.7), (3.5.8) or (3.5.11) according to the measurement scale of a given value.
...
$$
\lambda_j=\sum_{i=1}^n{\frac{d(x_{ij}^{\prime},x_{ij})}{n}}\in [0,1],    (3.5.13)
$$
$j=1,2,\ldots,m$.

An additional problem occurs if non-perturbative SDC tools are used. In this case the original values are either suppressed or remained unchanged. How to proceed in this case during computation of the measures (3.5.10) and (3.5.11) also depends on the measurement scale of the variables. The used If $X_j$ is nominal, then if $x_{ij}^{\prime}$ is hidden then one should assume $d(x_{ij}^{\prime},x_{ij})=1$; if $X_j$ is ordinal, then we assign $x_{ij}^{\prime}:=1$ if $x_{ij}$ is closer to $\mathfrak{r}_j$ or $x_{ij}^{\prime}:=\mathfrak{r}_j$ if $X_j$ is closer to 1; if $X_j$ is continuous, then
$$
x_{ij}^{\prime}:=\begin{cases}
\max\limits_{h=1,2,\ldots,n}{x_{hj}}&\textmd{if}\quad x_{ij}\le\med_{h=1,2,\ldots,n}{x_{hj}},\\
\min\limits_{h=1,2,\ldots,n}{x_{hj}}&\textmd{if}\quad x_{ij}>\med_{h=1,2,\ldots,n}{x_{hj}}.
\end{cases}
$$
...
In practice, the data holder (e.g. official statistics) prefers rather the first approach as the strict protection of data privacy is usually an obligation imposaed by valid law regulations. So, assurance of the safety of confidential information is very important.     
...
Hundepool, A., Van de Wetering, A., Ramaswamy, R., Franconi, L., Capobianchi, A., De Wolf, P.-P., Domingo-Ferrer, J., Torra, V., Brand, R:, and Giessing, S. (2005). *$\mu$-ARGUS version 4.0 Software and User's Manual*. Statistics Netherlands, Voorburg NL, may 2005. [[https://research.cbs.nl/casc]{.underline}](https://research.cbs.nl/casc/deliv/MUmanual4.0.pdf).
