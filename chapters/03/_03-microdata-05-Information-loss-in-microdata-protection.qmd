...
$$\text{dc}\left(c,c^'\right)=\frac{\left|c^{''}\text{:min}\left(c,c^'\right\leq c^{''}<\text{max}\left(c,c^'\right)\right|}{\left|D(V)\right|}$$ (3.5.2)
:::
...
$H\left(V|V^'=j\right)=-\sum_{i=1}^n{p\left(V=i|V^'=j\right)\text{log}p}\left(V=i|V^'=j|\right)$ (3.5.4)

The probabilities in Expression (3.5.4) can be derived from $P_{V,V^'}$ using Bayes's formula. Finally, the entropy-based information loss measure *EBIL* is obtained by accumulating Expression (3.5.4) for all individuals $r$ in the protected dataset $G$
...
$$
d({x_{ij}}^',x_{ij})=\begin{cases}
1&\textmd{if}\;x_{ij}}^'=x_{ij},\cr
0&\textmd{if}\;x_{ij}}^'\ne x_{ij}. (3.5.7)
$$
...
$$
d({x_{ij}}^',x_{ij})=\frac{\mathfrak{r}(x_{ij}}^',x_{ij})}{\mathfrak{r}_j-1}, (3.5.8)
$$
...
$$
\lambda=\sum_{j=1}^m{\sum_{i=1}^n{\frac{d(x_{ij}^',x_{ij})}{mn}}}\in [0,1],   (3.5.12)
$$
...
$$
\lambda_j=\sum_{i=1}^n{\frac{d(x_{ij}^',x_{ij})}{n}}\in [0,1],    (3.5.13)
$$
$j=1,2,\ldots,m$.
