...
An exception is the case when for some $n_0$ larger than 3 the agency thinks it is realistic to assume that a coalition of $n_0-2$ respondents contributing to the same cell may pool their data to disclose the contribution of another respondent. In such a case we set $n$ to $n_0$. Example 1c above provides an instance for this case with $n_0=$. (The intruder knows the pooled data of $5 - 2 = 3$ companies (e.g. B, C and D).
...
On the other hand the two largest contributions are $x_{1} + x_{2} = 102\ 000$. As $102\ 000 > \frac{100} {100 + p } \cdot X = 100\ 000$ the aggregate is not safe according to the $(2,k)$-rule.
...
\item $\frac{ \left( X - x_{2} \right) - x_{1} }{ X } \geq 1 - \frac{k}{100} = 1 - \frac{100}{100 + p} = \frac{p} {100 + p}$ (c.f. (3) ). Equal to (*ii*) is
...
| (n,k)-rule       	| $100 / k ⋅ (x_{1} + x_{2} + \cdots + x_{n}) − X$ 	|
| p%-rule          	|      $p / 100 ⋅ x_{1} − (X − x_{1} − x_{2})$     	|
| (p,q)-rule       	|       $p / q ⋅ x_{1} − (X − x_{1} − x_{2})$      	|
