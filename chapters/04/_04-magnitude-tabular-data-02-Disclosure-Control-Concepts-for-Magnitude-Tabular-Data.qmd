...
@tbl-sensitivity-rules briefly presents the most common sensitivity rules. Throughout this chapter we denote $x_{1} \geq x_{2} \geq \ldots \geq x_{N}$ the ordered contributions by respondents $1,2,\ldots,N$, respectively, to a cell with cell total (or cell value) $X = \sum\limits_{i \leq N}x_{i}$
  ...
  **$(n,k)$-dominance rule**   the sum of the $n$ largest contributions exceeds $k\%$ of the cell total, e.g. $x_{1} + \ldots + x_{n} > \frac{k}{100} \cdot X$ (1)

  **$p\%$-rule**                the cell total minus the 2 largest contributions $x_{1}$ and $x_{2}$ is less than $p\%$ of the largest contribution, e.g. $X - x_{2} - x_{1} < \frac{p}{100}x_{1}$[^5] (2)
  ---------------------------- ------------------------------------------------------------------------------------------------------------------------------------------------------------------------

: Sensitivity rules {#tbl-sensitivity-rules}

[^5]: $X − x_{n} − \cdots − x_{2} − x_{1} < \frac{p}{100} x_{1}$ for the case of coalitions of $n-1$ respondents, where $n>2$

Note that the dominance rule is not defined for $k=100$, neither is the $p\%$-rule for $p = 0$. Both are asymptotically equal to minimum frequency rules for $k\rightarrow 100$, or $p \rightarrow 0$ respectively.
...
An exception is the case when for some $n_0$ larger than 3 the agency thinks it is realistic to assume that a coalition of $n_0=2$ respondents contributing to the same cell may pool their data to disclose the contribution of another respondent. In such a case we set $n$ to $n_0=5$. Example 1c above provides an instance for this case with $n_0=$. (The intruder knows the pooled data of $5 - 2 = 3$ companies (e.g. B, C and D).
...
\item any aggregate, which is safe according to the\
$(2,k)$‑rule, is also safe according to the $p\%$-rule (this will be proven below), but

\item not any aggregate, which is safe according to the\
...
When we replace a $(2,k)$‑dominance rule, by a $p\%$‑rule, the natural choice is to derive the parameter $p$ from $k = 100 \cdot \frac{100}{100 + p}$ , e.g. to set $p = 100 \cdot \frac{100 - k}{k}$
...
Let $p = 10$, then $k = 100 \cdot \frac{100}{100 + p} = 90.9$, let the total value of a table cell be $X = 110 000$, let the largest two contributions be $x_{1} = 52\ 000$, and $x_{2} = 50 000$.

Then 

$$
\hat{x} = X - x_{2} = 110\ 000 − 50\ 000 = 60\ 000
$$
...
e.g. the upper estimate $\hat{x} = X - x_{2}$ will overestimate

the true value by $15.4\%$. So the aggregate is safe according to the $p\%$-rule at $p = 10$.

On the other hand the two largest contributions are $x_{1} + x_{2} = 102\ 000$. As $102\ 000 > \frac{100} {100 + p } \cdot X = 100\ 000$ the aggregate is not safe according to the $(2,k)$-rule. Example 1 provides illustrates this.

For an aggregate which is safe according to the $(2,k)$‑rule with $k = 100 \cdot \frac{100}{100 + p} \ldots$  (i)  and  (ii) will hold:

\begin{enumerate}[label=(\roman*)]
\item $x_{1} \leq \frac{k}{100} \cdot X = \frac{100}{100 + p} \cdot X$ and
\item $\frac{ \left( X - x_{2} \right) - x_{1} }{ X } \geq 1 - \frac{k}{100} = 1 - \frac{100}{100 + p} = \frac{p} {100 + p}$ (c.f. (3) ). 
\\ Equal to (ii) is
\item $\frac{ \left( X - x_{2} \right) - x_{1} }{ x_1 } \geq \frac{p}{100+p}\frac{X}{x1}$
\end{enumerate}


From (i) it follows that $\frac{ p }{ 100 + p } \cdot \frac{ X }{ x_{1} } \geq \frac{ p }{ 100 }$

And hence from (iii) : $\frac{ \left( X - x_{2} \right) -  x_{1} } { x_{1} } \geq \frac{p}{ 100 + p } \cdot \frac{ X }{ x_{ 1 } } \geq \frac{ p }{ 100 }$

A well known extension of the $p\%$-rule is the so called prior‑posterior $\text{(i)}\%$‑rule. With the extended rule, one can formally account for general knowledge about individual contributions assumed to be around *prior* to the publication, in particular that the second largest contributor can estimate the smaller contributions $X_{R} : = \sum\limits_{ i > 2 } x_{1}$ to within $q\%$. An aggregate is then considered unsafe when the second largest respondent could estimate the largest contribution $x_{1}$ to within $p$ percent of $x_{1}$ , by subtracting her own contribution and this estimate ${\hat{X}}_{R}$ from the cell total, e.g. when $|\left( X - x_{2} \right) - x_{1} - {\hat{X}}_{R}| < \frac{p}{100} \cdot x_{1}$. Because $\left( X - x_{2} \right) - x_{1} = X_{R}$, the left hand side is assumed to be less than $\frac{q}{100} \cdot X_{R}$. So the aggregate is considered to be sensitive, if $X_{R} < \frac{p}{q} \cdot x_{1}$. Evidently, it is actually the ratio $\frac{p}{q}$ which determines which cells are considered safe, or unsafe. Therefore, any $(p,q)$‑rule with $q < 100$ can also be expressed as $( p^* , q^*)$‑rule, with $q^* = 100$, e.g. choose

$$
p^* := 100 \cdot \frac{p}{q} \qquad \text{(4)}
$$ 

Of course we can also adapt the $(n,k)$-dominance rule to account for $q\%$ relative a priori bounds: Let e.g. $n = 2$ . According to (3) above, an aggregate should then be considered unsafe when the second largest respondent could estimate the largest contribution $x_{1}$ to within $(100 - k)$ percent of $X$ , by subtracting her own contribution and the estimate ${\hat{X}}_{R}$ from the cell total, e.g. when $|\left( X - x_{2} \right) - x_{1} - {\hat{X}}_{R}| < (100 - k)/100 \cdot X$. Just as in the case of the $p\%$-rule, we see that the aggregate is sensitive, when $X_{R} < \frac{100 - k}{q} \cdot X$, and that for given parameters $k$ and $q$ the parameter $k^*$ corresponding to $q^* = 100$ should be chosen such that

$$
\frac{100-k^*}{100} = \frac{100-k}{q} \qquad \text{(5)}
$$
...
\begin{itemize}
\item $X - x_{s} - x_{r} < \frac{p}{100}x_{s}$ , for the $p\%$-rule, and
\item $x_{r} + x_{s} > \frac{k}{100} \cdot X$ , for the $(2,k)$-dominance rule.
\end{itemize}
...
$$
\hat{X} - x_{2}^{s} - x_{1}^{s} < \frac{p}{100} \cdot x_{1}^{s} \qquad \text{(6)}
$$ 
...
$$
x_{1}^{s} + \ldots + x_{n}^{s} > \frac{k}{100} \cdot \hat{X} \qquad \text{(7)}
$$
...
$$
\hat{X} - w_{2} x_{2}^{s} - w_{1} x_{1}^{s} < \frac{p}{100} \cdot x_{1}^{s} \qquad \text{(6a)}
$$

where $w_{i}$, $i=1,2$, denote the sampling weights. Obviously, according to (6a) more aggregates will be sensitive.
...
with $X_{ij} \geq 0, \forall (i,j)$. Using linear programming methodology, it is possible to derive systematically for any suppressed cell in a table an upper bound $\left(X^{\max} \right)$ and a lower bound $\left( X_{11}^{\min} \right)$ for the set of feasible values. In the example above, for cell (1,1) these bounds are $\left( X_{11}^{\min} \right)$ = 3 and $X_{11}^{\max}$ = 6 .

In this simple instance, however, we do not need linear programming technology to derive this result: Because of the first column relation (C1) $X_{11}$ must be less or equal 6, and because of the second row relation (R2) $X_{21}$ must be less or equal 3. Therefore, and because of the first column relation (R1) $X_{11}$ must be at least 3.
