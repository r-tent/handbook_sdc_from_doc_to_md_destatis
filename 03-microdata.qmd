...
![](Images/media/image1.png)(3.2.3.1)
...
![](Images/media/image3.png). (3.2.5.1)
...
As defined in Skinner and Elamir (2004), assuming that the ![](Images/media/image9.png) are independently Poisson distributed with means ![](Images/media/image10.png) and assuming a Bernoulli sampling scheme with equal selection probably ![](Images/media/image11.png), then ![](Images/media/image12.png) and ![](Images/media/image13.png) are independently Poisson distributed as: ![](Images/media/image14.png) and ![](Images/media/image15.png) . The individual risk measure for a sample unique is defined as ![](Images/media/image16.png) which is equal to: ![](Images/media/image17.png) ![](Images/media/image18.png){width="1.575in" height="0.17222222222222222in"}

In this approach the parameters $\left\{ \lambda_{k} \right\}$ are estimated by taking into account the structure and dependencies in the data through log-linear modelling. Assuming that the sample frequencies $f_{k}$ are independently Poisson distributed with a mean of $u_{k} = \text{πλ}_{k}$, a log-linear model for the $u_{k}$ can be expressed as: $\text{log}(u_{k}) = x_{k}^{'}\beta$ where $x_{k}$ is a design vector denoting the main effects and interactions of the model for the key variables. Using standard procedures, such as iterative proportional fitting, we obtain the Poisson maximum-likelihood estimates for the vector $\beta$ and calculate the fitted values: ${\hat{u}}_{k} = \text{exp}(x_{k}^{'}\hat{\beta})$. The estimate for ${\hat{\lambda}}_{k}$ is equal to $\frac{{\hat{u}}_{k}}{\pi}$ which is substituted for ![](Images/media/image19.png) in the above formula for $r_{k}$. The individual disclosure risk measures can be aggregated to obtain a global (file-level) measure:
...
Let ![](Images/media/image28.png) be the matrix of the original data and ![](Images/media/image29.png) the matrix of continuous perturbation variables with expectation 1 and variance ![](Images/media/image30.png). The corresponding anonymised data ![](Images/media/image31.png) is then obtained as

> ![](Images/media/image32.png)

for each pair ![](Images/media/image33.png).

The following approach has been suggested by Höhne (2004). In a first step, for each record it is randomly decided whether its values are increased or decreased, each with 0.5-probability. This is done using the main factors ![](Images/media/image34.png) and ![](Images/media/image35.png). In order to avoid that all values of some record are perturbed with the same noise, these main factors are themselves perturbed with some additive noise ![](Images/media/image36.png). The following transformation is needed to preserve the first and second moments of the distribution:

> ![](Images/media/image37.png),

where ![](Images/media/image38.png) and ![](Images/media/image39.png) define the average of the original and anonymised variables, ![](Images/media/image40.png) and ![](Images/media/image41.png) the corresponding standard deviations, respectively.

Particularly if the original data follow a strongly skewed distribution, the deviations using this method may strongly depend on the configuration of the noise factors for some few, but large values. That is, despite consistency, means and sums might be unsatisfactorily reproduced. For this reason, (Höhne, 2004) suggests a slight modification of the method. At first, we generate normal distributed random variables ![](Images/media/image42.png) with expectation greater than zero and \'small\' variance, s.t. the realisation of ![](Images/media/image43.png) yields a positive value. Afterwards, the data is sorted in descending order by the variable under consideration. Then, the record with the largest entry in this variable is diminished by

> ![](Images/media/image44.png).

The records ![](Images/media/image45.png) are now perturbed as follows:

> ![](Images/media/image46.png)
>
> ![](Images/media/image47.png).

Hence, means and sums are preserved and the diminishing and enlarging effects of single values cancel out each other. For the remaining record ![](Images/media/image48.png) we set
...
![](Images/media/image136.png)Figure 3‑3: Overview of μ‑ARGUS
