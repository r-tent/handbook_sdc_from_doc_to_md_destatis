...
Assuming that there are no coalitions of respondents, i.e. there are no intruders knowing more than one of the contributions. Then the closest upper estimate of any other contribution can be obtained by the second largest contributor, when it subtracts its own contribution $x_{2}$from the aggregate total (e.g. cell value) *X* to estimate the largest contribution (${\hat{x}}_{1} = X - x_{2}$) as seen in example 1. All other scenarios of a contributor subtracting its own value from the total, to estimate another, result in larger error. In the, rather unlikely, scenario that ![](Images/media/image138.png) respondents pool their data in order to disclose the contribution of another, the closest upper estimate of any other contribution can be obtained by the coalition of respondents 2, 3,...*n* when they estimate ${\hat{x}}_{1} = X - x_{2} - x_{3} - \text{.}\text{.}\text{.} - x_{n}$.
...
In this formulation, the (2*,k*)-rule looks very similar to the formulation of the *p*%-rule given by (2). Both rules define an aggregate to be sensitive, when the estimate ![](Images/media/image139.png) does not overestimate the true value of ![](Images/media/image140.png)'sufficiently'. The difference between both rules is in how they determine this 'sufficiency'. According to the *p* %-rule, it is expressed as a rate (i.e. *p *%) of the true value of the largest contribution ![](Images/media/image141.png), while according to the (2,*k*) ‑rule, it is expressed as a rate (i.e. *100-k* %) of the aggregate total *X*. Considering this, the concept of the *p*%-rule seems to be more natural than that of the (2,*k*)-rule.

(2,*k*)-rules correspond to *p%*-rules in the following way:

If *k* is set to ![](Images/media/image142.png) then
...
Let *p* = 10, then ![](Images/media/image143.png) = 90.9, let the total value of a table cell be *X* = 110 000 , let the largest two contributions be ![](Images/media/image144.png) = 52 000 , and ![](Images/media/image145.png) = 50 000 .

Then ![](Images/media/image146.png) = 110 000 − 50 000 = 60 000 

![](Images/media/image147.png){width="1.4159722222222222in" height="0.4465277777777778in"}![](Images/media/image148.png){width="1.5298611111111111in" height="0.4465277777777778in"}\
e.g. the upper estimate ![](Images/media/image149.png) will overestimate

the true value by 15.4 %. So the aggregate is safe according to the *p* %-rule at *p* = 10.

On the other hand the two largest contributions are ![](Images/media/image150.png)= 102 000. As 102 000  \> ![](Images/media/image151.png) = 100 000  the aggregate is not safe according to the (2,*k*)-rule.

own contribution from the aggregate total. Example 1 provides an instance.

For an aggregate which is safe according to the (2,*k*)‑rule with ![](Images/media/image152.png)

will hold:

*(i)* ![](Images/media/image153.png) , and

*(ii)* ![](Images/media/image154.png) (c.f. (3) ). Equal to (*ii*) is

\(iii\) ![](Images/media/image155.png){width="1.832638888888889in" height="0.4465277777777778in"}

From (*i*) it follows that ![](Images/media/image156.png) .

And hence from (*iii*) : ![](Images/media/image157.png) .

A well known extension of the *p* %--rule is the so called prior‑posterior (i)%‑rule. With the extended rule, one can formally account for general knowledge about individual contributions assumed to be around *prior* to the publication, in particular that the second largest contributor can estimate the smaller contributions ![](Images/media/image158.png) to within *q* %. An aggregate is then considered unsafe when the second largest respondent could estimate the largest contribution ![](Images/media/image159.png) to within *p* percent of ![](Images/media/image160.png) , by subtracting her own contribution and this estimate ${\hat{X}}_{R}$ from the cell total, e.g. when $|\left( X - x_{2} \right) - x_{1} - {\hat{X}}_{R}| < \frac{p}{\text{100}} \cdot x_{1}$. Because $\left( X - x_{2} \right) - x_{1} = X_{R}$, the left hand side is assumed to be less than $\frac{q}{\text{100}} \cdot X_{R}$. So the aggregate is considered to be sensitive, if $X_{R} < \frac{p}{q} \cdot x_{1}$. Evidently, it is actually the ratio *p/q* which determines which cells are considered safe, or unsafe. Therefore, any (*p,q*)‑rule with *q* \< 100 can also be expressed as ![](Images/media/image161.png)‑rule, with ![](Images/media/image162.png), e.g. choose

> $p^{} = :\text{100} \cdot p/q$ (4)

Of course we can also adapt the (*n,k*)-dominance rule to account for *q* % relative a priori bounds: Let e.g. *n* = 2 . According to (3) above, an aggregate should then be considered unsafe when the second largest respondent could estimate the largest contribution $x_{1}$ to within (100 -- *k*) percent of *X* , by subtracting her own contribution and the estimate ${\hat{X}}_{R}$ from the cell total, e.g. when $|\left( X - x_{2} \right) - x_{1} - {\hat{X}}_{R}| < (\text{100} - k)/\text{100} \cdot X$. Just as in the case of the *p%*-rule, we see that the aggregate is sensitive, when $X_{R} < (\text{100} - k)/q \cdot X$, and that for given parameters *k* and *q* the parameter *k\** corresponding to ![](Images/media/image163.png) should be chosen such that
...
When disclosure risk has to be assessed for a variable that can take not only positive, but also negative values, we suggest to reduce the value of *p* (or increase *k*, for the dominance-rule, resp.). It may even be adequate to take that reduction even to the extent of replacing a concentration rule by a minimum frequency rule. This recommendation is motivated by the following consideration: Above, we have explained that the *p%*‑rule is equivalent to a (*p,q*)-rule with *q *= 100. When contributions may take negative, as well as positive values, it makes sense to assume that the bound *q~-~* for the relative deviation of *a priori* estimates ${\hat{X}}_{R}$ exceeds 100 %. This can be expressed as ![](Images/media/image164.png)*,* with ![](Images/media/image165.png). According to (4) the *p* parameter *p~f~* for the case of negative/positive data should be chosen as ![](Images/media/image166.png)*.* That means particularly that for large *f* the *p*%-rule with corresponding parameter ![](Images/media/image167.png) is asymptotically equal to the minimum frequency rule (c.f. the remark just below table 1).

In case of the dominance rule, because of (4) , ![](Images/media/image168.png) can be determined by ![](Images/media/image169.png), and because ![](Images/media/image170.png) this means that ![](Images/media/image171.png). For large *f* the dominance rule with parameter ![](Images/media/image172.png) will be asymptotically equal to the minimum frequency rule.

Sometimes, respondents authorize publication of an aggregate even if this publication might cause a risk of disclosure for their contributions. Such authorizations are also referred to as '*waivers*'. When *s* is the largest respondent from which no such waiver has been obtained, and *r* is the largest respondent except for *s* then for any pair *(i,j*), ![](Images/media/image173.png) of respondents, where no waiver has been obtained from *j*, it holds $x_{i} + x_{j} \leq x_{r} + x_{s}$. Therefore, we will be able to deal with such a situation properly, if we reformulate the concentration rules as
...
> ![](Images/media/image174.png) (6a),

where ![](Images/media/image175.png) (i=1,2) denote the sampling weights. Obviously, according to (6a) more aggregates will be sensitive.
...
Using linear programming methodology, it is possible to derive systematically for any suppressed cell in a table an upper bound ![](Images/media/image177.png) and a lower bound ![](Images/media/image178.png) for the set of feasible values. In the example above, for cell (1,1) these bounds are ![](Images/media/image179.png) = 3 and ![](Images/media/image180.png) = 6 .

In this simple instance, however, we do not need linear programming technology to derive this result: Because of the first column relation (C1) ![](Images/media/image181.png) must be less or equal 6, and because of the second row relation (R2) ![](Images/media/image182.png) must be less or equal 3. Therefore, and because of the first column relation (R1) ![](Images/media/image183.png) must be at least 3.
...
![](Images/media/image185.png)Figure 4‑4: Overview of the τ‑ARGUS architecture
...
In this way, when processing table T*i* (*i* = 1,2,3) in iteration step *k~0~* ,we consider as additional suppressions $\underset{k < k_{0}}{}\left( S_{1\text{ki}} \cup S_{2\text{ki}} \cup S_{3\text{ki}} \right) \cup \underset{j < i}{}S_{\text{jk}_{0}i}$ , while we may try to avoid (by weighting, or by setting cell status to 'protected') that those cells that the tables have in common, i.e. the cells of ![](Images/media/image186.png), and that are not already contained in the set of additional suppressions are selected as secondary suppressions. Note that we assign 0 % protection levels to cells in *S~iki~*, the set of secondary suppression which have been assigned to the current table in a previous step *k* of the iteration. The process is continued until for each of the tables the set of additional suppressions that has to be considered when processing the table in this step is identical to the set considered in the previous step.
...
Network flow heuristics for secondary cell suppression build on the fact, that a suppressed cell in a two-dimensional table is safe from exact disclosure if, and only if, the cell is contained in a 'cycle', or 'alternating path' of suppressed cells, where a cycle is defined to be a sequence of non-zero cells with indices $\left\{ (i_{0}\text{,j}_{0})\text{,\ }(i_{1}\text{,j}_{0})\text{,\ }(i_{1}\text{,j}_{1})\text{,\ }(i_{2}\text{,\ j}_{1}),\text{.}\text{.}\text{.},(i_{n}\text{,j}_{n})\text{,\ }(i_{0}\text{,j}_{n}) \right\}$, where all ![](Images/media/image187.png) and ![](Images/media/image188.png) for *k* = 0,1,\...,*n* , and *l* = 0,1,\....,*n* respectively (*n*: length of the path), are different, e.g. $i_{k_{l}}$ ≠ $i_{k_{2}}$ unless $k_{1}$ = $k_{2}$ , and $j_{l_{1}}$ ≠ $\ j_{l_{2}}$ unless $l_{1}$ = $l_{2}$ .
